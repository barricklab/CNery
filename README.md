...
- `CNV_out/OTR_corr/` — per-reference OTR bias plots and a JSON summary (`*_otr_results.json`) containing the inferred origin window, terminus window, normalized coverage at each, the origin-to-terminus ratio, and the sequence's **relative copy number**.
  ...
  When a large amplification or deletion is detected in the fitted ramp's residual, it is reported as `"Event p-value"` / `"Event start (bp)"` / `"Event end (bp)"` and censored from a single refit of the origin, terminus and amplitude — real copy-number events otherwise pull the ramp toward themselves, and the fitted ramp then divides part of a real duplication back out of the data. Detection never changes whether a ramp was found or which estimate supplied it; those are decided before any censoring, so the procedure cannot talk itself into a correction. The reported interval is approximate: it is no sharper than the narrowest width the scan considers, so an event much smaller than that is bracketed rather than delimited.
