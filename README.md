...
- `CNV_out/OTR_corr/` — per-reference OTR bias plots and a JSON summary (`*_otr_results.json`) containing the inferred origin window, terminus window, normalized coverage at each, the origin-to-terminus ratio, and the sequence's **relative copy number**.
  ...
  When a large copy-number change or inversion is detected in the fitted ramp's residual, it is reported as `"Event kind"` / `"Event start (bp)"` / `"Event end (bp)"` and censored from a single refit of the origin, terminus and amplitude — real events otherwise pull the ramp toward themselves. Detection never changes whether a ramp was found or which estimate supplied it; those are decided before any censoring, so the diagnostic cannot talk itself into a correction. Note inversions are visible only through the replication ramp itself (they mirror it), so they are undetectable without one, and an inversion centred on the origin produces no coverage change at all.
